...
def shuffle_exp(
    ...
    with torch.no_grad():
        for i in range(batch_size // 5):
            ...
            if i == 0:
                out = llama.model(cur_batch_input_ids)[0]
            else:
                out = torch.concat([out, llama.model(cur_batch_input_ids)[0]])

    res, indices = pred_probs(out, llama.lm_head.weight, correct_next_token_ids)
        ...
        for _ in range(10):
            ...
            with torch.no_grad():
                for i in range(batch_size // 5):
                    cur_batch_input_ids = input_ids.cuda()[i * 5 : i * 5 + 5]
                    if i == 0:
                        out = model_edit(cur_batch_input_ids)[0]
                    else:
                        out = torch.concat([out, llama.model(cur_batch_input_ids)[0]])
            ...
            if n in probs:
                probs[n] = np.vstack([probs[n], res[:, T_cnt:-1, 2]])
            else:
                probs[n] = res[:, T_cnt:-1, 2]
            ...
            if n in errs:
                errs[n] = np.vstack(
                    [
                        errs[n],
                        indices[:, T_cnt:-1, 0]
                        != correct_next_token_ids[:, T_cnt:-1:].numpy(),
                    ]
                )
            else:
                errs[n] = (
